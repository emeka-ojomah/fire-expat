...
import api from '../api';
import { AuthShell, Card, BrandMark } from './ui/Shell';
...
export default function Login({ setIsAuthenticated }) {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!token) return toast.error('Login failed: no session returned.');
    ...
    <AuthShell>
      <Card className="p-7 sm:p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <Input icon={Mail} label="Email Address" type="email" placeholder="you@example.com"
        ...
        <p className="text-center text-sm text-slate-500 mt-5">
          No account? <Link to="/register" className="text-indigo-400 font-semibold hover:text-indigo-300">Create one free</Link>
        </p>
        <div className="flex flex-wrap justify-center gap-2 mt-5">
          {['Multi-currency', 'FIRE Calculator', 'Expat-focused'].map((b) => (
            <span key={b} className="text-[10px] text-slate-500 border border-slate-800 rounded-full px-2.5 py-1">{b}</span>
          ))}
        </div>
      </Card>
    </AuthShell>
