/**
 * Single source of truth for the fire-expat logo + wordmark.
 * Used by both the dashboard Sidebar and the auth pages (Login/Register/ProfileSetup)
 * so they can never drift apart.
 *
 * size="sm"  → desktop sidebar (compact, no tagline)
 * size="lg"  → auth pages (larger, with tagline)
 */
export default function BrandMark({ size = 'sm', withTagline = false }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        className={`${boxClass} bg-indigo-600/15 border border-indigo-500/30 flex items-center justify-center shrink-0`}
      >
